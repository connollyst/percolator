...
import org.dosbcn.percolator.notifications.RandomTimeTestStatistics;
...
public class AssertTime {
    ...
    /**
     * Assert that the {@link RandomTimeTestStatistics} reflect an even distribution.<br/>
     * The lower and upper limits should be respected but, given the large number of random times sampled, we expect
     * the lowest and highest times seen to be close to those limits. We also assert that the mean of all the sampled
     * times is roughly half way between the lower and upper limits.
     *
     * @param stats
    ...
    public static void assertEvenDistribution(RandomTimeTestStatistics stats,
        ...
        assertBetween("lower limit", lowerInterval, actualMinimum);
        assertBetween("upper limit", upperInterval, actualMaximum);
    ...
    public static void assertBefore(String message, DateTime expected, DateTime actual) {
        assertTrue(message + ": expected " + actual + " to be before " + expected,
                actual.isBefore(expected));
    }

    public static void assertAfter(String message, DateTime expected, DateTime actual) {
        assertTrue(message + ": expected " + actual + " to be after " + expected,
                actual.isAfter(expected));
    }

    public static void assertBetween(String message, Interval expected, DateTime actual) {
