...
public class RandomTimeGenerator {
     ...
     * If there is time to send a notification today, a random time between now and the cutoff is chosen. If it is too
     * late to send a notification today, a random time tomorrow is returned instead.
     *
     ...
     * Note: It is possible <i>now</i> is later than the earliest notification time, usually used in generating a
     * random time. If this is the case, we need to generate a time after now, not the notification time so as not to
    ...
    /**
     * The actual random time generator: generates a random time, in milliseconds, between two cutoffs, in
     * milliseconds.
     */
    private final class RandomTime {
        ...
        private static final int EARLIEST_NOTIFICATION_MS = EARLIEST_NOTIFICATION_HOUR * HOUR_MS;
        private static final int LATEST_NOTIFICATION_MS = LATEST_NOTIFICATION_HOUR * HOUR_MS;
         ...
         */
        public int nextTime() {
            return nextTimeBetween(EARLIEST_NOTIFICATION_MS, LATEST_NOTIFICATION_MS);
        }

        /**
         * Return a random time (milliseconds in day) <i>after</i> the limit specified, within the hard limits.
         *
         * @param msEarliest
         *         an early cutoff
         * @return a random time in the day
        ...
        /**
         * Return a random time (milliseconds in day) <i>before</i> the limit specified, within the hard limits.
         *
         * @param msLatest
         *         an late cutoff
         * @return a random time in the day
        ...
        /**
         * Return a random time (milliseconds in day) <i>between</i> the two limits specified, within the hard limits.
         *
         * @param msEarliest
         *         an early cutoff
         * @param msLatest
